...
function replacer(key, value): any {
	...
	if(typeof value === "lotus.domino.local.DateTime" || value instanceof java.util.Date) {
		let jd = value instanceof java.util.Date ? value : value.toJavaDate();
...
function getDocItems(doc: NotesDocument, items) {
	var o = {};
	for(var k in items) {
		o[k] = util.vector2array(doc.getItemValue(k));
		if(o[k].length <= 1) o[k] = o[k].length == 0 ? "" : o[k][0];
	}
	return o
}
...
export function stream(param: any, out:java.io.OutputStream, headers: any, response: any): any {
	...
	if(!!param.parentColumns) {
		parentColumns = {};
		let temp = param.parentColumns.split(",");
	...
	if(!!param.computedColumns) {
		let temp = fromJson(param.computedColumns);
		for(var i = 0; i < temp.length; i++) {
			let o = temp[i];
	...
	var cols: string[] = [];
	var colNames: { [key: string]: number} = {};
	...
	//print("before writer")
	var writer: java.io.OutputStreamWriter = new java.io.OutputStreamWriter(out, "UTF-8");
	//print("after writer");
	...
	if(resultCount > 0) {
		...
		let toWrite = serializeEntryColumns(nve,cols, colNames ,dbPath,parentColumns, computedColumns, search, searchIn);
